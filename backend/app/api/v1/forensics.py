...
router = APIRouter(prefix="/v1", tags=["forensics"], dependencies=[Depends(require_role("admin", "investigator"))])
...
async def forensics_network(db: AsyncSession = Depends(get_db)) -> dict:
    return await get_actor_target_network(db)
